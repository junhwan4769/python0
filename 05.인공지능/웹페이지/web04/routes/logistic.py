...
def predict_data():
    ...
    return df.to_html(classes='table table-striped table-dark table-hover', index=False)

@bp.route('/')
def logistic():
    import pandas as pd
    df = pd.read_csv('data/로지스틱회귀.csv')
    table = df.to_html(classes='table table-striped table-dark table-hover', index=False)
